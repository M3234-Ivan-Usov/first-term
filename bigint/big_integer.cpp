...
big_integer &big_integer::operator-=(big_integer const &rhs) {
    ...
    big_integer substractor = rhs.absolute();
    bool result_sign = sign;
    sign = false;
    if (*this < substractor) {
        result_sign = !result_sign;
        std::swap(*this, substractor);
    }
    int64_t carry = 0;
    int64_t next = 0;
    for (size_t i = 0; i < substractor.value.size() || carry; ++i) {
        next = value[i] - (carry + substractor.get_or_default(i));
        carry = 0;
        if (next < 0) {
            carry = 1;
            next += _2_power_32;
...
big_integer &big_integer::operator*=(big_integer const &rhs) {
    ...
    for (size_t i = 0; i < old.value.size(); ++i) {
        for (size_t j = 0; j < rhs.value.size() || carry; j++) {
            ...
            value[i + j] = static_cast<uint32_t>(next & UINT32_MAX);
            carry = static_cast<uint32_t>(next >> BITS_IN_CELL);
        }
    }
    trim();
    sign = old.sign ^ rhs.sign;
    return *this;
}
...
void big_integer::difference(big_integer const &dq, uint64_t k, uint64_t m) {
    int64_t borrow = 0;
    int64_t diff = 0;
    const int64_t b = static_cast<int64_t>(_2_power_32);
...
std::pair<big_integer, big_integer> big_integer::normalise(big_integer const &b, size_t m) {
    auto f = _2_power_32 / (static_cast<uint64_t>(b.value[m - 1]) + 1);
